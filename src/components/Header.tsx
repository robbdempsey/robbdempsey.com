...
import { navLinks, NavLinkItem } from '@/lib/navLinks'
...
function MobileNavigation(
  ...
  function getMobileNavItem(nav: NavLinkItem) {
    return (
      <MobileNavItem key={`mobilenavitem-${nav.href}`} href={nav.href}>{nav.title}</MobileNavItem>
    )
  }
      ...
      >
        ...
        <nav className="mt-6">
          <ul className="-my-2 divide-y divide-zinc-100 text-base text-zinc-800 dark:divide-zinc-100/5 dark:text-zinc-300">
            {navLinks.map(item => getMobileNavItem(item))}
          </ul>
        </nav>
      </PopoverPanel>
    </Popover>
...
function DesktopNavigation(props: React.ComponentPropsWithoutRef<'nav'>) {
  ...
  function getNavItem(nav: NavLinkItem) {
    return (
      <NavItem key={`mobilenavitem-${nav.href}`}  href={nav.href}>{nav.title}</NavItem>
    )
  }
  ...
  return (
    <nav {...props}>
      <ul className="flex rounded-full bg-white/90 px-3 text-sm font-medium text-zinc-800 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10">
        {navLinks.map(item => getNavItem(item))}
      </ul>
    </nav>
  )
}
